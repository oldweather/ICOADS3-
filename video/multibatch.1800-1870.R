...
while(current.day<=end.day) {
  ...
  cat('#!/bin/ksh -l\n')
  cat('#SBATCH --output=/scratch/hadpb/slurm_output/ICOADS3p-%j.out\n')
  ...
  while(this.job<per.job) {
     cat(sprintf("./ICOADS_3v3+_single.R --year=%d --month=%d --day=%d --hour=0 --spread=72\n",
                 year(current.day),month(current.day),day(current.day)))
     cat(sprintf("./ICOADS_3v3+_single.R --year=%d --month=%d --day=%d --hour=12 --spread=72\n",
                 year(current.day),month(current.day),day(current.day)))
